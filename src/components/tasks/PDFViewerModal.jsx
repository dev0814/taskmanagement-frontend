...
const PDFViewerModal = ({ isOpen, onClose, docId, taskId, filename, documentUrl }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // If documentUrl is provided directly (from Cloudinary), use it
      if (documentUrl) {
      ...
      if (token) {
        // First try to get the document info to check if it's a Cloudinary URL
        fetch(`${api.defaults.baseURL}/tasks/${taskId}`, {
          headers: {
        ...
        .then(data => {
          const document = data.data.documents.find(doc => doc._id === docId);
          ...
          if (document && document.url) {
            ...
            return fetch(`${api.defaults.baseURL}/tasks/${taskId}/documents/${docId}`, {
              headers: {
                'Authorization': `Bearer ${token}`
              }
            });
        ...
        .then(response => {
          if (!response) return; // Skip if we already set the URL from Cloudinary
          if (!response.ok) {
        ...
        .then(blob => {
          if (!blob) return; // Skip if we already set the URL from Cloudinary
  ...
  }, [isOpen, docId, taskId, token, documentUrl]);
  ...
  const handleDownload = () => {
    // For Cloudinary URLs, open in a new tab
    if (pdfUrl && pdfUrl.includes('cloudinary.com')) {
      window.open(pdfUrl, '_blank');
      return;
    }
    
    // For blob URLs (local files), trigger download
